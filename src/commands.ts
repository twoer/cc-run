...
export async function list() {
  intro(pc.cyan(pc.bold('⚡ ccx-run list')))

  const { providers, source } = await loadProviders()

  if (providers.length === 0) {
    log.warn('No providers found')
    outro(pc.dim('Run `ccx-run add` to add one'))
    return
  }
...
export async function add() {
  intro(pc.cyan(pc.bold('⚡ ccx-run add')))
...
export async function rm() {
  intro(pc.cyan(pc.bold('⚡ ccx-run rm')))
...
export async function edit() {
  intro(pc.cyan(pc.bold('⚡ ccx-run edit')))
