...
function showHelp() {
  console.log(`
  ${pc.cyan(pc.bold('⚡ ccx-run'))} ${pc.dim(`v${VERSION}`)}
  ${pc.dim('Claude Code launcher')}

  ${pc.bold('Usage:')} ccx-run [command] [options] [provider-name]
  ...
  ${pc.bold('Examples:')}
    ${pc.dim('$')} ccx-run              ${pc.dim('# Interactive select, current terminal')}
    ${pc.dim('$')} ccx-run glm          ${pc.dim('# Fuzzy match provider name')}
    ${pc.dim('$')} ccx-run --new        ${pc.dim('# Interactive select, new window')}
    ${pc.dim('$')} ccx-run add          ${pc.dim('# Add a new provider')}
    ${pc.dim('$')} ccx-run list         ${pc.dim('# List all providers')}
    ${pc.dim('$')} ccx-run edit         ${pc.dim('# Edit a provider')}
    ${pc.dim('$')} ccx-run rm           ${pc.dim('# Remove a provider')}
...
export async function run(argv: string[]) {
  ...
  intro(`${pc.cyan(pc.bold('⚡ ccx-run'))} ${pc.dim('— Claude Code eXecutor')}`)
  const { providers, source } = await loadProviders()
  log.message(
    pc.dim(`${providers.length} providers from ${source || 'none'} · v${VERSION}\n`) +
    pc.dim(`  ccx-run ${pc.cyan('add')}   Add provider    ccx-run ${pc.cyan('edit')}  Edit provider\n`) +
    pc.dim(`  ccx-run ${pc.cyan('list')}  List providers   ccx-run ${pc.cyan('rm')}    Remove provider\n`) +
    pc.dim(`  ccx-run ${pc.cyan('-n')}    New window       ccx-run ${pc.cyan('help')}  Show help`),
  )

  if (providers.length === 0) {
    log.error('No providers found')
    log.message('')
    log.message(`  ${pc.cyan('1.')} Run ${pc.bold('ccx-run add')} to add a provider`)
